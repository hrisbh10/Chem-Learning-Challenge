...
x_norm = np.concatenate((x_norm,traindt[['Weekday','Weekend']]),axis=1)
tst_norm = np.array(dat_norm[classes.shape[0]:])
tst_norm = np.concatenate((tst_norm,testdt[['Weekday','Weekend']]),axis=1)
...
#     tf.keras.layers.Dense(14,input_shape=(x_norm.shape[1],),kernel_regularizer=tf.keras.regularizers.l2(lda),activation='relu'),
#     tf.keras.layers.Dense(10,kernel_regularizer=tf.keras.regularizers.l2(lda-0.01),activation='relu'),
#     tf.keras.layers.Dense(8,kernel_regularizer=tf.keras.regularizers.l2(lda+0.02),activation='relu'),
#     tf.keras.layers.Dense(5,kernel_regularizer=tf.keras.regularizers.l2(lda-0.01),activation='softmax')
# ])
...
clf.fit(x_norm,classes)
y_pred = clf.predict(tst_norm)
print("Accuracy = {}".format(accuracy_score(class_test,y_pred)))
print("F1 score = {}".format(f1_score(class_test,y_pred,average='micro')))
print("Kappa score = {}".format(cohen_kappa_score(class_test,y_pred)))
print("Confusion Matrix :")
